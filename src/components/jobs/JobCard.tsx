...
export default function JobCard({vacancy, grid}: {vacancy: VacancyInterface, grid: boolean}) {
    ...
    return (
        <button className={"border border-gray-400 rounded-xl p-4 " +
            "transition-ease-in duration-300 hover:shadow-lg relative"}
                style={{width: grid ? "300px" : "100%"}}>
            <button onClick={() => {
                saveVacancy(vacancy);
                setSaved(!saved)
            }}
                    className={`flex justify-center items-center w-8 h-8 rounded-full absolute top-0 right-0 -translate-x-1/4 translate-y-1/3`}>
                <div
                    >
                ...
                {grid ? (
                    <div className={"flex items-center text-center"}>
                        <div className={"flex flex-col"}>
                            ...
                            <button
                                className={"mt-4 border border-blue-900 rounded-md bg-blue-100 hover:bg-blue-800 hover:text-white transition-ease-in duration-300"}>
                    ...
                    <div className={"flex flex-col items-start justify-start w-full relative"}>
                        <div className="grid grid-cols-4 gap-6 items-start justify-start w-full">
                            ...
                            <button
                                className={"absolute bottom-0 right-0 w-44 p-1.5 bg-red-100 border border-red-700 rounded-md hover:bg-red-700 transition-ease-in duration-300"}>
                                <p className={"text-red-900 font-semibold hover:text-white"}>Apply Now!</p>
                            </button>
                        </div>
                    </div>
                )}
